...
: ```R
?functionName       # Help section on function
??TermOfInterest    # Find functions and features matching a term of interest
args(functionName)  # List arguments that the function accepts
``` 
...
:  {.borderless .sm tbl-colwidths="[50,50]" }
...
: ```R
x <- 4.5
x <- y <- z <- 4.5     # Assign to multiple variables
x <<- "global value"   # Assign/change value of variable in global scope 
```
